...
server <- function(input, output, session) {
  ...
  output$distPlot <- renderPlot({
    ...
    validate(
      need(input$Se != 1 & input$Sp != 1, 
      "Sorry, in this simulation dichotomous Se and Sp have to be smaller tan 1.")
    )
  ...
  output$overlap = renderText(paste("Overlap Distributions =", 
      2*((1-input$pretestprob/100)*(1-input$Se)+
           (input$pretestprob/100)*(1-input$Sp)), '%'))
